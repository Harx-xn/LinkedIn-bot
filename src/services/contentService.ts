...
export class ContentService {
    ...
    async generatePost(topic: string, articleLink: string, provider: 'GEMINI' | 'OPENAI' = 'GEMINI', tone: string = 'Professional'): Promise<any> {
        const prompt = `
      You are a professional LinkedIn content creator for InnovariaTech, an IT consulting and technology services company.
      ...
      POSITIONING:
      - InnovariaTech helps businesses modernize their tech stack
      - We focus on: core software systems, secure infrastructure, automation, data visibility
      - Target audience: business leaders, CTOs, IT decision-makers
      - Topics: IT consulting, digital transformation, enterprise software, Web3/crypto infrastructure
      ...
      Output MUST be valid JSON:
      {
        ...
        "hashtags": "#DigitalTransformation #ITConsulting #TechStrategy #InnovariaTech #EnterpriseIT"
      }
    ...
    async generateMixedPost(trends: { topic: string, link: string }[], provider: 'GEMINI' | 'OPENAI' = 'GEMINI', tone: string = 'Professional'): Promise<any> {
        const topicsList = trends.map(t => `"${t.topic}"`).join(' and ');
        const references = trends.map(t => `- ${t.topic}: ${t.link}`).join('\n');

        const prompt = `
      You are a professional LinkedIn content creator for InnovariaTech.
      ...
      OUTPUT MUST BE VALID JSON:
      {
        ...
        "hashtags": "#TechTrends #InnovariaTech #${trends[0].topic.replace(/ /g, '')}"
      }
